...
- (void)layoutSubviews
{
    ...
    self.arrowImage.frame = CGRectMake(0, 5, self.width, 10);
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state)
    {
        ...
        {
            // 设置文字
            self.statusLabel.text = NSLocalizedString(@"上拉可以加载更多数据", @"MJRefresh");
...
//                    self.arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
        ...
        {
            // 设置文字
            self.statusLabel.text = NSLocalizedString(@"松开立即加载更多数据", @"MJRefresh");
...
//                self.arrowImage.transform = CGAffineTransformIdentity;
        ...
        {
            // 设置文字
            self.statusLabel.text = NSLocalizedString(@"正在帮你加载数据...", @"MJRefresh");

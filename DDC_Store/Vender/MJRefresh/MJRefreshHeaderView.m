...
- (void)updateTimeLabel
{
    ...
    // 1.获得年月日
    NSCalendar *calendar1 = [NSCalendar respondsToSelector:@selector(calendarWithIdentifier:)] ? [NSCalendar calendarWithIdentifier:NSCalendarIdentifierGregorian] : [NSCalendar currentCalendar];
    NSUInteger unitFlags = NSCalendarUnitYear| NSCalendarUnitMonth | NSCalendarUnitDay |NSCalendarUnitHour |NSCalendarUnitMinute;
    NSDateComponents *cmp1 = [calendar1 components:unitFlags fromDate:_lastUpdateTime];
    NSDateComponents *cmp2 = [calendar1 components:unitFlags fromDate:[NSDate date]];
    ...
    if(time.length==5)
    {
        time =[NSString stringWithFormat:@"%@%@",NSLocalizedString(@"今天", @"MJRefreshHeaderView"),time];
    }
    // 3.显示日期
    self.lastUpdateTimeLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新：%@", @"MJRefreshHeaderView"), time];
}
...
- (void)setState:(MJRefreshState)state
{
	...
	switch (state) {
        ...
        {
            // 设置文字
			self.statusLabel.text = NSLocalizedString(@"上拉可以刷新", @"MJRefresh");
...
//                    self.arrowImage.transform = CGAffineTransformIdentity;
        ...
        {
            // 设置文字
            self.statusLabel.text = NSLocalizedString(@"松开立即刷新", @"MJRefresh");
            // 执行动画
            [UIView animateWithDuration:MJRefreshFastAnimationDuration animations:^{
//                self.arrowImage.transform = CGAffineTransformMakeRotation(M_PI);
        ...
        {
            // 设置文字
            self.statusLabel.text = NSLocalizedString(@"正在帮你刷新...", @"MJRefresh");

...
#import "DDCQRCodeAlbumManager.h"
#import <AVFoundation/AVFoundation.h>
#import <Photos/Photos.h>
#import "UIImage+ImageSize.h"

#ifdef DEBUG
#define DDCQRCodeLog(...) NSLog(__VA_ARGS__)
#else
#define DDCQRCodeLog(...)
#endif

@interface DDCQRCodeAlbumManager () <UINavigationControllerDelegate, UIImagePickerControllerDelegate>
@property (nonatomic, strong) UIViewController *currentVC;
@property (nonatomic, strong) NSString *detectorString;
@end

@implementation DDCQRCodeAlbumManager

+ (instancetype)sharedManager {
    static DDCQRCodeAlbumManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[DDCQRCodeAlbumManager alloc] init];
    });
    return manager;
}
...
- (void)readQRCodeFromAlbumWithCurrentController:(UIViewController *)currentController {
    ...
    if (currentController == nil) {
        NSException *excp = [NSException exceptionWithName:@"DDCQRCode" reason:@"readQRCodeFromAlbumWithCurrentController: 方法中的 currentController 参数不能为空" userInfo:nil];
        [excp raise];
    }
        ...
        if (status == PHAuthorizationStatusNotDetermined) { // 用户还没有做出选择
            ...
            [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
                if (status == PHAuthorizationStatusAuthorized) { // 用户第一次同意了访问相册权限
                    ...
                    if (self.isOpenLog) {
                        DDCQRCodeLog(@"用户第一次同意了访问相册权限 - - %@", [NSThread currentThread]);
                    }
                } else { // 用户第一次拒绝了访问相机权限
                    if (self.isOpenLog) {
                        DDCQRCodeLog(@"用户第一次拒绝了访问相机权限 - - %@", [NSThread currentThread]);
                    }
                }
            }];
            ...
            self.isPHAuthorization = YES;
            if (self.isOpenLog) {
                DDCQRCodeLog(@"访问相机权限 - - %@", [NSThread currentThread]);
            }
            [self enterImagePickerController];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    if (features.count == 0) {
        ...
        for (int index = 0; index < [features count]; index ++) {
            ...
            if (self.isOpenLog) {
                DDCQRCodeLog(@"相册中读取二维码数据信息 - - %@", resultStr);
            }

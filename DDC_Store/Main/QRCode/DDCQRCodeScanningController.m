...
@implementation DDCQRCodeScanningController
...
- (void)dealloc {
    NSLog(@"DDCQRCodeScanningVC - dealloc");
...
- (DDCQRCodeScanningView *)scanningView {
    if (!_scanningView) {
        _scanningView = [[DDCQRCodeScanningView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 0.9 * self.view.frame.size.height)];
        //        _scanningView.scanningImageName = @"DDCQRCode.bundle/QRCodeScanningLineGrid";
...
- (void)QRCodeAlbumManager:(DDCQRCodeAlbumManager *)albumManager didFinishPickingMediaWithResult:(NSString *)result {
#warning 待修改
...
- (void)QRCodeScanManager:(DDCQRCodeScanManager *)scanManager didOutputMetadataObjects:(NSArray *)metadataObjects {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        [scanManager palySoundName:@"DDCQRCode.bundle/sound.caf"];
...
- (UIButton *)flashlightBtn {
    if (!_flashlightBtn) {
        ...
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"DDCQRCodeFlashlightOpenImage"] forState:(UIControlStateNormal)];
        [_flashlightBtn setBackgroundImage:[UIImage imageNamed:@"DDCQRCodeFlashlightCloseImage"] forState:(UIControlStateSelected)];
...
- (void)flashlightBtn_action:(UIButton *)button {
    if (button.selected == NO) {
        [DDCQRCodeHelperTool DDC_openFlashlight];
...
- (void)removeFlashlightBtn {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [DDCQRCodeHelperTool DDC_closeFlashlight];
